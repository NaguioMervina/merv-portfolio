...
- The portfolio includes skills, featured projects, experience history, GitLab and GitHub contribution snapshots, and contact details.
...
- The portfolio shows GitLab and GitHub contribution sections with summary metrics and heatmaps.
- Contribution metric cards use the full fetched provider range from first contribution through the build date when provider data is available.
- Contribution heatmaps show the most recent 365-day window through the build date to keep the UI compact and scannable.
- GitLab data is fetched during `npm run build` and written to `resources/js/data/gitlab-contributions.json`.
- GitHub data is fetched during `npm run build` and written to `resources/js/data/github-contributions.json` when a build-time `GITHUB_TOKEN` is configured.
- The deployed site renders generated snapshots and does not need a database or runtime backend for contribution data.
- If GitLab or GitHub is unavailable during build, the portfolio still builds and shows an unavailable state for that provider.
...
- Frontend: JavaScript, React.js, Vue.js, HTML/CSS/Bootstrap
- Database: MySQL, Database Design
- Tools: Git/GitHub/GitLab, Docker, Jira, WSL Ubuntu, phpMyAdmin, PuTTY, MySQL Workbench
...
- Runtime server-side GitLab or GitHub scraping or contribution caching
